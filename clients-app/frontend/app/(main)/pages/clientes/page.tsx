...
const ClientesPage = () => {
    ...
    const fetchClientes = async () => {
        try {
            const response = await axios.get("http://localhost:8086/api/clientes");
    ...
    const saveCliente = async () => {
        ...
        if (cliente.nombre.trim() && cliente.correo.trim()) {
            try {
                let response;
                if (cliente.id) {
                    response = await axios.put(`http://localhost:8086/api/clientes/${cliente.id}`, cliente);
                    toast.current?.show({ severity: 'success', summary: 'Éxito', detail: 'Cliente actualizado', life: 3000 });
                } else {
                    response = await axios.post('http://localhost:8086/api/clientes', cliente);
    ...
    const deleteCliente = async () => {
        try {
            await axios.delete(`http://localhost:8086/api/clientes/${cliente.id}`);
