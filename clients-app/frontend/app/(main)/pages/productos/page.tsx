...
const ProductoPage = () => {
    ...
    const fetchProductos = async () => {
        try {
            const response = await axios.get("http://localhost:8086/api/productos");
    ...
    const saveProducto = async () => {
        ...
        if (producto.nombre.trim() && producto.precio > 0) {
            try {
                if (producto.id) {
                    await axios.put(`http://localhost:8086/api/productos/${producto.id}`, producto);
                    toast.current?.show({ severity: "success", summary: "Éxito", detail: "Producto actualizado", life: 3000 });
                } else {
                    const response = await axios.post("http://localhost:8086/api/productos", producto);
    ...
    const deleteProducto = async () => {
        try {
            await axios.delete(`http://localhost:8086/api/productos/${producto.id}`);
